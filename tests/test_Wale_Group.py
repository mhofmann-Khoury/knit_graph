...
class TestWale_Group(TestCase):
    ...
    def test_single_loop_wale_in_groups(self):
        ...
        terminal_groups = set(w.terminal_loop for w in wale_groups)
        for loop in knit_graph.terminal_loops():
            self.assertIn(loop, terminal_groups, f"Expected terminal loop {loop} to be in wale_groups")
    ...
    def test_mesh(self):
        ...
        for wg in wale_groups:
            if len(wg.wale_graph.nodes) == 1:
                if len(wg) != 2:
                    self.assertEqual(len(wg), 6, f"Expected single wale of 2 or 6")
            else:
